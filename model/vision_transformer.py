...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x1, attn_weight = self.attn(self.norm1(x))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # print(f'embed_len: {embed_len}')    # 197
        self.pos_embed = nn.Parameter(torch.randn(1, embed_len, embed_dim) * .02)
        # print(f'self.pos_embed: {self.pos_embed.shape}')    # # 197
    ...
    def load_pretrained(self, checkpoint_path: str, prefix: str = '') -> None:
        print(f'99999999999999999999999999')
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = '') -> None:
    ...
    if big_vision:
        ...
        # print(f'None---big_vision')
        pos_embed_w = _n2p(w[f'{prefix}Transformer/posembed_input/pos_embedding'], t=False)
    if pos_embed_w.shape != model.pos_embed.shape:
        print('----------------------------pos_embed_w.shape != model.pos_embed.shapen')
    ...
    # print(f'***********************pos_embed_w = {pos_embed_w.shape}')  # [1, 197, 384]
    # print(f'***********************model.pos_embed.data.shape = {model.pos_embed.data.shape}')  # [1, 396, 384]
    ...
    if model.attn_pool is not None:
        # print('----------------------------model.attn_pool is not None')
    ...
    for i, block in enumerate(model.blocks.children()):
        print('----------------------------enumerate(model.blocks.children())')
